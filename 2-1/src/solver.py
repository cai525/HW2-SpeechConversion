...
from src.model import SpeakerClassifier
from src.model import PatchDiscriminator
import os
from util.utils import Logger
from util.utils import to_var
from util.utils import reset_grad
from util.utils import grad_clip
from util.utils import cal_acc
from util.utils import cc
from util.utils import calculate_gradients_penalty
from util.utils import Hps

class Solver(object):
    def __init__(self, hps_tuple, data_loader, log_dir='./log/'):
        self.patch_opt = None
        self.gen_opt = None
        self.clf_opt = None
        self.ae_opt = None
        self.PatchDiscriminator = None
        self.SpeakerClassifier = None
        self.Generator = None
        self.Decoder = None
        self.Encoder = None
        self.hps = hps_tuple
    ...
    def save_model(self, model_path, iteration=0, enc_only=True):
        if not enc_only:
            all_model = {
    ...
    def permute_data(self, data):
        ...
        """
        C = torch.tensor(data[0], requires_grad=False,dtype=torch.long).to("cuda")
    ...
    def train(self, model_path, flag='train', mode='train'):
        ...
        if mode == 'pretrain_G':
            ...
            for iteration in range(self.last_iter,hps.patch_iters):
                # =======train D=========#
            ...
            for iteration in range(self.last_iter,hps.iters):
                ...
                # decode
                x_tilde = self.decode_step(enc, c)
                loss_rec = torch.mean(torch.abs(x_tilde - x))       # auto-encoder 的误差
                # classify speaker
                logits = self.clf_step(enc)
                acc = cal_acc(logits, c)
                loss_clf = self.cal_loss(logits, c)
                ...
                loss.backward()
                grad_clip([self.Encoder, self.Decoder], self.hps.max_grad_norm)
                self.ae_opt.step()      # ae equals to auto encoder
                info = {
